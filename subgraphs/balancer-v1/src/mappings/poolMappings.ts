...
export function handleSetSwapFee(event: LOG_CALL): void {
  ...
  let pool = getOrCreateLiquidityPool(poolId, event.block);
...
export function handleSetCrpController(event: OwnershipTransferred): void {
  ...
  if (crpPool !== null) {
    let pool = getOrCreateLiquidityPool(crpPool, event.block);
...
export function handleSwap(event: LOG_SWAP): void {
  ...
  let tokenAmountOut = event.params.tokenAmountOut;
  let newAmountOut = pool.inputTokenBalances
    .at(tokenOutIdx)
