...
import { ERC20 } from "../../../../generated/DODOZoo/ERC20";
import { ERC20SymbolBytes } from "../../../../generated/DODOZoo/ERC20SymbolBytes";
import { ERC20NameBytes } from "../../../../generated/DODOZoo/ERC20NameBytes";
import {
...
export function getOrCreateProtocol(): DexAmmProtocol {
  let dodoZoo = DexAmmProtocol.load(CLASSIC_FACTORY_ADDRESS);
  if (!dodoZoo) {
    dodoZoo = new DexAmmProtocol(CLASSIC_FACTORY_ADDRESS);
    dodoZoo.name = "Dodo V1";
    dodoZoo.slug = "dodo-v1";
...
export function createLiquidityPool(
  ...
  baseLpTokenAddress: Address,
  quoteLpTokenAddress: Address,
  timestamp: BigInt,
  blockNumber: BigInt,
  lpFeeRate: BigDecimal,
  name: string
  ...
  pair.protocol = getOrCreateProtocol().id;
  let baseToken = getOrCreateToken(baseTokenAddress, timestamp);
  let quoteToken = getOrCreateToken(quoteTokenAddress, timestamp);
  if (!baseLpTokenAddress.equals(Address.empty())) {
  ...
  if (!quoteLpTokenAddress.equals(Address.empty())) {
  ...
  let dodoZoo = getOrCreateProtocol();
  dodoZoo.totalPoolCount += 1;
  DODOTemplate.create(pairAddress);
  dodoZoo.save();
  return pair;
}
...
export function getOrCreateToken(address: Address, timestamp: BigInt): Token {
  ...
  if (!token) {
    ...
    let dodoZoo = getOrCreateProtocol();
    dodoZoo._tokenCount = dodoZoo._tokenCount.plus(ONE_BI);
    dodoZoo.save();
  }
