...
import { Conversion } from "../../generated/templates/Converter/ConverterBase";
...
export function createConverterBaseReturnFunctions(
  ...
  createMockedFunction(address, "version", "version():(uint16)").returns([
  ...
  ).returns([ethereum.Value.fromUnsignedBigInt(BigInt.fromString(type))]);
...
// export function createContractRegistry(address: string, contractAddress: string, name: Bytes): AddressUpdate {
//   const event = changetype<AddressUpdate>(newMockEvent())
//   event.parameters = new Array();
//   let pa0 = new ethereum.EventParam(
//       "_contractAddress",
//       ethereum.Value.fromAddress(Address.fromString(contractAddress))
//   );
//   let pa1 = new ethereum.EventParam(
//       "_contractName",
...
export function createConverterAnchorAdded(
  address: string,
  _anchor: string,
  txFrom: string,
  txTo: string,
  ...
  const event = changetype<ConverterAnchorAdded>(newMockEvent());
  event.parameters = new Array();
  let pa0 = new ethereum.EventParam(
    "_anchor",
    ethereum.Value.fromAddress(Address.fromString(_anchor))
  );
  event.parameters.push(pa0);
